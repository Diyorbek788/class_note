...
class NoteViewSet(ViewSet):
    ...
    def create(self, request):
        ...
        if title and body and owner_id:
            note = Note.objects.create(title=title, body=body, owner_id=owner_id)
            note.save()
            serializer = NoteSerializer(instance=note, many=False)
            return Response(serializer.data, status=status.HTTP_201_CREATED)
        else:
            return Response({"message": "Title, Body, and Owner ID are required to create a note"},
                            status=status.HTTP_400_BAD_REQUEST)
    ...
    def update(self, request, pk):
        ...
        if note:
            ...
            if title and body and owner_id:
                ...
                return Response({"detail": "Title, Body, and Owner ID are required"},
                                status=status.HTTP_400_BAD_REQUEST)
        else:
            return Response({"detail": "Note not found"}, status=status.HTTP_404_NOT_FOUND)
